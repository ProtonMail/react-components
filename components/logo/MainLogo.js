...
import { useUser, useSubscription, useOrganization, Href } from 'react-components';

import { formatPlans } from '../../containers/payments/subscription/helpers';

const MainLogo = ({ product, url = 'https://mail.protonmail.com/' }) => {
    const [{ isFree, hasPaidMail, hasPaidVpn } = {}, loadingUser] = useUser();
    const [{ Theme } = {}, loadingOrganization] = useOrganization();
    const [{ Plans } = {}, loadingSubcription] = useSubscription();
    ...
    const display = product === 'vpn' ? hasPaidVpn : hasPaidMail;
    const plan = product === 'vpn' ? vpnPlan.Name : mailPlan.Name;

    return loadingUser && loadingOrganization && loadingSubcription ? null : (
        <Href url={url} rel="noreferrer help" className="logo-container nodecoration flex-item-centered-vert">
            <svg
...
MainLogo.propTypes = {
    product: PropTypes.oneOf(['mail', 'contacts', 'calendar', 'drive', 'vpn']),
